...
app.use(function(req, res, next){
 ...
 app.get("/addfield",async (req,res) => {
    var players = await Player.updateMany({},{$set:{'amountPaid':0}});
     res.send("success");
 })


 app.get("/",async (req,res) => {
    var players=await Player.find({willAttend:true}).sort({matches:-1,name:1});
    var lostCount=0;
    var absentCount=0;
    var winCount = 0;
    ...
    var playersAttend = 0;
    players.forEach((p) => {
        lostCount = lostCount + p.lost;
        absentCount = absentCount + p.absent;
        winCount = winCount + p.won;
        amountCollected = amountCollected + p.amountPaid;
        if(p.willAttend == true){
            playersAttend = playersAttend + 1;
        }
    });
    var tcm = lostCount * 15 + winCount*5 + absentCount*10;
...
app.get("/verysecureone",async (req,res) => {
        var players=await Player.find({willAttend:true}).sort({lost:-1,name:1});
        var lostCount=0;
        var absentCount=0;
        var winCount = 0;
        var amountCollected=0;
        ...
        var tcm = lostCount * 15 + winCount*5 + absentCount*10;
        res.render('landing',{players:players,tcm:tcm,acm:amountCollected,totalPlayers:totalPlayers,playersAttend:playersAttend});
});

app.get("/verysecureoneall",async (req,res) => {
    var players=await Player.find({}).sort({lost:-1,name:1});
    var lostCount=0;
    var amountCollected=0;
    var totalPlayers = players.length;
    var playersAttend = 0;
...
app.get("/handleAbsent/:pid",async (req,res) => {
...
app.get("/handlePay/:id",async (req,res) => {
    ...
    player.amountPaid = player.lost * 10;
    player.won=0;
    player.lost=0;
    player.matches=0;
    await player.save();
...
app.get("/handleCustomPay/:id",async (req,res) => {
    res.render("custompay",{id:req.params.id});
})

app.post("/handleCustomPay/:id",async (req,res) => {
    console.log(typeof req.body.amount);
    var player = await Player.findOne({_id:req.params.id});
    player.amountPaid = req.body.amount;
    player.won=0;
    player.lost=0;
    player.matches=0;
    await player.save();
...
app.get("/handleAttend",async (req,res) => {
    var players = await Player.updateMany({},{$set:{'willAttend':true}});
    res.send("success");
})


app.get("/willNotAttend/:id",async (req,res) => {
    var player = await Player.findOne({_id:req.params.id});
    player.willAttend= false;
    player.amountPaid = 0;
    player.won=0;
    player.lost=0;
    player.matches=0;
...
app.get("/willAttend/:id",async (req,res) => {
    var player = await Player.findOne({_id:req.params.id});
    player.willAttend= true;
    player.amountPaid = 0;
    player.won=player.history.won;
    player.lost=player.history.lost;
    player.matches=player.history.matches;
    await player.save();
    console.log(player);
    res.redirect("/verysecureone");
});
...
app.get("/updateBattingStats",async (req,res) => {
    var data=[
        ...
        { _id: '621073ce0222251b60374fb3', name: 'Alam Bhai', runs: 11,ballsPlayed:18 },
        { _id: '620f1c8a74916272450ed3f0', name: 'Ali', runs: 0,ballsPlayed:0 },
        { _id: '620be3bea9261146e0f043a6', name: 'Aquib',runs: 1,ballsPlayed:0 },
        ...
        { _id: '620be3bea9261146e0f043a5', name: 'Buni', runs: 6,ballsPlayed:6},
        { _id: '620be3bea9261146e0f043a7', name: 'Chotu',runs: 0,ballsPlayed:2 },
        { _id: '620c4ad74b18020bbe14c392', name: 'Dablu Bhai',runs: 1,ballsPlayed:4},
        { _id: '620be3bea9261146e0f043af', name: 'Danish', runs: 9,ballsPlayed:6 },
        ...
        { _id: '620be3bea9261146e0f043a4', name: 'Fayeque',runs: 2,ballsPlayed:2 },
        { _id: '620be3bea9261146e0f043ac', name: 'Irfan', runs: 7,ballsPlayed:10 },
        { _id: '6211c550aaf67c752e3f0e81', name: 'Majhi bhai', runs: 0,ballsPlayed:0 },
        { _id: '6220bea6b83bc74780d9ef04', name: 'Munna Bhai', runs: 0,ballsPlayed:2 },
        { _id: '620be3bea9261146e0f043b1', name: 'Paale', runs: 5,ballsPlayed:5 },
        ...
        { _id: '620be3bea9261146e0f043ae', name: 'Saddam Bhai', runs: 26,ballsPlayed:17 },
        { _id: '620be3bea9261146e0f043a3', name: 'Saif Ali',runs: 7,ballsPlayed:11 },
        { _id: '620f0af674916272450ed3ef', name: 'Samir',runs: 24,ballsPlayed:18},
        { _id: '6220be8ab83bc74780d9ef03', name: 'Samir Bhai',runs: 2,ballsPlayed:4  },
        { _id: '620c52be4b18020bbe14c396', name: 'Shakil Bhai',runs: 0,ballsPlayed:0 },
        { _id: '620be3bea9261146e0f043a8', name: 'Sheru Bhai',runs: 6,ballsPlayed:0 },
        { _id: '6222db5ca4017a3f0c34e5b6', name: 'Shivam',runs: 0,ballsPlayed:0  },
        { _id: '620be3bea9261146e0f043a9', name: 'Sohrab Bhai',runs: 46,ballsPlayed:22 },
        { _id: '6221963d3db48d0ec800e57b', name: 'Tannu',runs: 0,ballsPlayed:3 },
        ...
        { _id: '620be3bea9261146e0f043b6', name: 'Wajid Bhai', runs: 12,ballsPlayed:8 },
        { _id: '620c52a54b18020bbe14c395', name: 'Zeeshan Bhai', runs: 0,ballsPlayed:0 }
      ]
    ...
    app.get("/updateNew",async (req,res) => {
        var data=[
           
            { _id: '6222db5ca4017a3f0c34e5b6', name: 'Shivam',runs: 8,ballsPlayed:3  },
...
app.get("/updateBowlingStats",async (req,res) => {
    ...
    var data=[
        { _id: '620be3bea9261146e0f043b8', name: 'Akil Bhai',runsGiven: 17,ballsDelivered:12,wickets:2},
        { _id: '620be3bea9261146e0f043a5', name: 'Buni',runsGiven: 31,ballsDelivered:24,wickets:1 },
        ...
        { _id: '620be3bea9261146e0f043a4', name: 'Fayeque',runsGiven: 12,ballsDelivered:18,wickets:2},
        { _id: '6211c550aaf67c752e3f0e81', name: 'Majhi bhai',runsGiven: 0,ballsDelivered:0,wickets:0 },
        { _id: '6220bea6b83bc74780d9ef04', name: 'Munna Bhai',runsGiven: 22,ballsDelivered:0,wickets:0},
        { _id: '620c527d4b18020bbe14c393', name: 'Rajji Bhai',runsGiven: 24,ballsDelivered:18,wickets:1 },
        { _id: '620be3bea9261146e0f043b0', name: 'Sabbir Bhai',runsGiven: 11,ballsDelivered:12,wickets:2},
        { _id: '620be3bea9261146e0f043ae', name: 'Saddam Bhai',runsGiven: 30,ballsDelivered:24,wickets:1 },
        { _id: '620be3bea9261146e0f043a3', name: 'Saif Ali',runsGiven: 18,ballsDelivered:18,wickets:2 },
        { _id: '6220be8ab83bc74780d9ef03', name: 'Samir Bhai',runsGiven: 30,ballsDelivered:18,wickets:2 }, 
        { _id: '620be3bea9261146e0f043a8', name: 'Sheru Bhai',runsGiven: 1,ballsDelivered:0,wickets:0},
...
app.get("/battingAverage",async (req,res) => {
    var data=await Player.find({innings : {$gt:5}}).sort({battingAverage:-1});
    res.render("battingAverage",{data:data});
});

app.get("/bowlingWickets",async (req,res) => {
    var data=await Player.find({wickets : {$gt : 0}}).sort({wickets:-1,economy:1});
    res.render("bowlingWickets",{data:data});
});

app.get("/testing",async (req,res) => {
    var p = await Player.find({willAttend:true}).sort({name:1});
    var totalDue=0;
    var dueEach=0;
    p.forEach((d) => {
        dueEach=d.lost*10 + (15-d.matches)*10;
        console.log(d.name + 'will have to pay  ' + dueEach);
        totalDue += d.lost*10 + (12-d.matches)*10;
        dueEach=0;
    })
    console.log(totalDue);
    res.send("Total due is" + (totalDue + 450));
})


app.get("/resetAmount",async (req,res) => {
    var players=await Player.updateMany({},{$set:{amountPaid:0}});
    res.send("success");
})
app.get("/addHistoryAbsent",async (req,res) => {

   var players=await Player.updateMany({},{$set:{'history.absent':0}});
    res.send("success");
})
